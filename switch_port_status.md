...
### Current blocker: the render texture FBO is incomplete

With the joystick out of the way the sequence reaches:

```
video: create window 1280x720
video: surface ready (refs 2)
video: show window / show window done
egl: creating context / egl: context created
[[SFML ERROR]]: Impossible to create render texture
                (failed to link the target texture to the framebuffer)
```

So window creation, the EGL surface and the GL context **all work** for the real render
window. The next blocker is VRSFML's `RenderTexture`: `glCheckFramebufferStatus` comes back
incomplete.

Known about it so far:

- **Not resolution related.** Fails identically at 1280x720 and 1920x1080.
- The engine requests `stencilBits = 8` in its `ContextSettings`, so the depth/stencil
  attachment format is the first thing to check — nouveau may not accept the combination
  VRSFML picks by default.
- Worth logging the actual `glCheckFramebufferStatus` value and the attachment formats;
  the current message does not say which attachment is at fault.

**The game is therefore not playable yet.** It starts, mounts its assets, brings up GL and
creates its window, and stops at the first render target.
