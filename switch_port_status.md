...
**Leading hypothesis:** it waits on a window event this backend never produces.
`SWITCH_PumpEvents` only polls `appletMainLoop()`; it never sends `SDL_EVENT_WINDOW_SHOWN`
or `..._EXPOSED`. That is the first thing to try.
...
The remaining `SDL_GetJoystickVendor` warning is benign: the joystick backend reports no
vendor id.
