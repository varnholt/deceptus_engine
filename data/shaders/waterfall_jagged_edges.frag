...
void main()
{
    ...
    // Stronger edge fade: Reduce brightness and alpha near the sides
    float edge_factor = smoothstep(0.0, 0.15, uv.x) * smoothstep(1.0, 0.85, uv.x);
    color.a *= mix(1.0, 0.0, edge_noise * (1.0 - edge_factor));
    color.rgb *= edge_factor; // Darken near edges
