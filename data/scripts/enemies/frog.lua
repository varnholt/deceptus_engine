...
_tongue_extension = 0.0  -- Current tongue extension scale (0.0 to 5.0)
...
function updateSprite(dt)
   ...
   -- add direction offset to the row
   if not _points_left then
      cycle = cycle + ROW_OFFSET_RIGHT
   end
   ...
   local frame_idx = math.floor(_animation_frame) % max_frames

   -- calculate sprite coordinates
   x = frame_idx * SPRITE_WIDTH
   ...
   if _state == STATE_IDLE then
      handleIdleState(frame_idx)
   elseif _state == STATE_ATTACK then
      handleAttackState(frame_idx)
   end

end

-- function to handle idle state updates
function handleIdleState(frame_idx)
...
function handleAttackState(frame_idx)
   ...
   local show_tongue = frame_idx >= 2 and frame_idx <= 4  -- show tongue during middle frames of attack
   ...
   if show_tongue then
      ...
      if _tongue_extension == 0.0 then
         _tongue_extension = 0.0  -- start at 0
      end
      _tongue_extension = _tongue_extension + dt * 10  -- increment by dt * 10
      if _tongue_extension > 5.0 then
         _tongue_extension = 5.0  -- clamp at 5
      end
      
      -- apply the scale
      setSpriteScale(1, _tongue_extension, 1.0)  -- scale only horizontally
      
      -- show the tongue sprite
      setSpriteVisible(1, true)
      
      -- calculate the actual displayed tongue width based on scale
      local displayed_tongue_width = 24 * _tongue_extension  -- original width (24) scaled by current scale
...
function initialize()
   ...
   setSpriteVisible(1, false)  -- hide tongue initially
