...
|randomize_offset_px|int|Constant shift in pixels added on top of the randomized hit position. Use a negative value to center the range around the surface, e.g. `randomize_range_px` of `4` together with `randomize_offset_px` of `-2` scatters the splashes by 2 pixels in either direction. The offset is clamped to `randomize_range_px` in both directions, since a larger offset would move every single splash off the surface. The default is `0`.|
