...
|randomize_offset_px|int|Constant shift in pixels added on top of the randomized hit position. Use a negative value to move the range up so it straddles the surface, e.g. `randomize_range_px` of `4` together with `randomize_offset_px` of `-2` scatters the splashes between 2 pixels above and 1 pixel below the surface. The offset is clamped to `randomize_range_px` in both directions, so a mistyped value cannot push the whole scatter more than one range away from the surface. The default is `0`.|
