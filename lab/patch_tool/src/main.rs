...
impl PatchApp {
    fn new() -> Self {
        let patches_dir = locate_catacombs_dir();
        let available_patches = patches_dir.as_deref().map(scan_for_patches).unwrap_or_default();
        Self {
            patches_dir,
            available_patches,
            selected_index: 0,
    ...
    fn refresh_patch_list(&mut self) {
        self.available_patches =
            self.patches_dir.as_deref().map(scan_for_patches).unwrap_or_default();
    ...
    fn apply_selected_patch(&mut self) {
        ...
        let Some(ref patches_dir) = self.patches_dir.clone() else {
            self.append_log("Patches directory not found.\n");
            return;
        };
        ...
        for file_patch in &file_patches {
            ...
            let report = apply_patch_to_dir(patches_dir, file_patch);
        ...
        if overall_success {
            let applied_dir = patches_dir.join("applied_patches");
...
impl eframe::App for PatchApp {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            ui.horizontal(|ui| {
                ui.label("Folder:");
                match &self.patches_dir {
                    Some(directory) => {
                        ui.monospace(directory.to_string_lossy().as_ref());
                    }
                    None => {
                        ui.colored_label(egui::Color32::RED, "data/catacombs not found");
                    }
                }
            });
            ...
            let can_apply = !self.available_patches.is_empty() && self.patches_dir.is_some();
            if ui
...
/// looking for a `data/catacombs/` subdirectory (project root marker).
fn locate_catacombs_dir() -> Option<PathBuf> {
    // Prefer the working directory — most reliable when run from the project root.
    if let Ok(working_dir) = std::env::current_dir() {
        let candidate = working_dir.join("data").join("catacombs");
        if candidate.is_dir() {
            return Some(candidate);
        }
    }

    // Fall back to walking up from the executable path.
    let exe_path = std::env::current_exe().ok()?;
    let mut search_dir = exe_path.parent()?;
    for _ in 0..12 {
        let candidate = search_dir.join("data").join("catacombs");
        if candidate.is_dir() {
            return Some(candidate);
        }
...
/// Collect all `.diff` and `.patch` files from `directory`, sorted by name.
fn scan_for_patches(directory: &Path) -> Vec<PathBuf> {
    let Ok(read_dir) = fs::read_dir(directory) else {
        return Vec::new();
    };

    let mut patch_files: Vec<PathBuf> = read_dir
        .filter_map(|entry| {
            ...
            (extension == "diff" || extension == "patch").then_some(path)
        })
        .collect();
