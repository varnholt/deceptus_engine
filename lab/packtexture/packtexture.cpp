...
void PackTexture::pack()
{
  ...
  for (auto blockY = 0; blockY < sizeY; blockY++)
  {
    ...
    {
      updateProgress_(100 * (static_cast<float>(progress)/progressDone));
      progress++;
...
void PackTexture::dump()
{
  ...
  std::cout << "quad count: " << count << std::endl;
  ...
  std::cout << "picking a " << textureSize_ << "x" << textureSize_ << " texture" << std::endl;
  ...
  for (auto& quad : quads_)
  {
    ...
    updateProgress_(100 * (static_cast<float>(progress)/quads_.size()));
