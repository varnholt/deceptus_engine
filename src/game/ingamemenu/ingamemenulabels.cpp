...
)
{
   ...
   MenuLabel::compose(
      *layer,
      ...
      {MenuLabel::KeptRegion{._source = sf::IntRect{{0, 0}, {icon_width_px, height_px}}, ._target = sf::Vector2i{0, 0}}},
      {MenuLabel::Label{
         ._text = source_text,
...
void InGameMenuLabels::updateHeaderLabels(Layer& header_layer, Tab selected, int32_t pill_left_px, int32_t pill_width_px)
{
   ...
   {
      widest_label_px = std::max(widest_label_px, MenuLabel::measureWidth(tab_name, character_size));
   }
   ...
   const auto kept_regions = MenuLabel::stretchedPlate(
      sf::IntRect{{pill_left_px, 0}, {pill_width_px, height_px}}, pill_x_px, new_pill_width_px, pill_cap_width_px
   );
   ...
   MenuLabel::compose(header_layer, {image_width_px, height_px}, kept_regions, labels);
...
void InGameMenuLabels::updateFooterLabels(const std::vector<FooterHint>& hints)
{
   ...
   {
      const auto item_width_px = hint._icon_width_px + hint_icon_gap_px + MenuLabel::measureWidth(hint._text, character_size);
