...
bool Init(sf::Window& window, const sf::Vector2f& displaySize, bool loadDefaultFont)
{
   ...
   const auto loadMouseCursor = [](ImGuiMouseCursor imguiCursorType, sf::Cursor::Type sfmlCursorType)
   {
      ...
      if (cursor_opt.hasValue())
         s_currWindowCtx->mouseCursors[imguiCursorType] = std::move(*cursor_opt);
   };
#else
   const auto loadMouseCursor = [](ImGuiMouseCursor imguiCursorType, sf::Cursor::Type sfmlCursorType)
...
bool UpdateFontTexture()
{
   ...
   newTexture_opt->update(pixels);
   ImTextureID texID = convertGLTextureHandleToImTextureID(newTexture_opt->getNativeHandle());
   io.Fonts->SetTexID(texID);
   s_currWindowCtx->fontTexture = std::move(*newTexture_opt);
#else
...
bool ImageButton(const char* id, const sf::Texture& texture, const sf::Vector2f& size, const sf::Color& bgColor, const sf::Color& tintColor)
{
   ImTextureID textureID = convertGLTextureHandleToImTextureID(texture.getNativeHandle());
...
void DrawLine(const sf::Vector2f& a, const sf::Vector2f& b, const sf::Color& color, float thickness)
{
   ...
   draw_list->AddLine(
      ImVec2(a.x + pos.x, a.y + pos.y), ImVec2(b.x + pos.x, b.y + pos.y), ColorConvertFloat4ToU32(toImColor(color)), thickness
   );
}
...
void DrawRect(const sf::FloatRect& rect, const sf::Color& color, float rounding, int rounding_corners, float thickness)
{
   ImDrawList* draw_list = ImGui::GetWindowDrawList();
   draw_list->AddRect(
      getTopLeftAbsolute(rect), getDownRightAbsolute(rect), ColorConvertFloat4ToU32(toImColor(color)), rounding, rounding_corners, thickness
   );
}
...
void DrawRectFilled(const sf::FloatRect& rect, const sf::Color& color, float rounding, int rounding_corners)
{
   ...
   draw_list->AddRectFilled(
      getTopLeftAbsolute(rect), getDownRightAbsolute(rect), ColorConvertFloat4ToU32(toImColor(color)), rounding, rounding_corners
   );
}
...
void updateJoystickButtonState(ImGuiIO& io)
{
   ...
   for (int i = 0; i < static_cast<int>(sf::Joystick::ButtonCount); ++i)
   {
      ...
      {
         const bool isPressed = joystick_query->isButtonPressed(static_cast<unsigned>(i));
...
void updateMouseCursor(sf::Window& window)
{
   ...
   if ((io.ConfigFlags & ImGuiConfigFlags_NoMouseCursorChange) == 0)
   {
      ...
      else
      {
         ...
         const sf::Cursor& c = s_currWindowCtx->mouseCursors[cursor] ? *s_currWindowCtx->mouseCursors[cursor]
